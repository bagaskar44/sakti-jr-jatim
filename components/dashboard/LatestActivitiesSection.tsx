...
export function LatestActivitiesSection({
  ...
  if (!selectedUnit) {
    return (
      <div
        className={`flex h-full min-h-[400px] flex-col items-center justify-center border-t border-[#dce3ed] p-6 text-center 2xl:border-l 2xl:border-t-0 ${className}`}
      >
        <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-[7px] bg-blue-50 text-[#1f4fea]">
          <MapPinned size={24} />
        </div>
        <p className="text-sm font-bold text-slate-950">Pilih marker unit</p>
        <p className="mt-2 max-w-[270px] text-xs font-semibold leading-5 text-slate-500">
  ...
  return (
    <div
      className={`h-full min-h-[400px] border-t border-[#dce3ed] pt-4 2xl:border-l 2xl:border-t-0 2xl:pl-4 2xl:pt-0 ${className}`}
    >
      <div className="mb-3 flex min-h-7 items-start justify-between gap-3">
        ...
        <Link
          ...
          prefetch={false}
          className="inline-flex shrink-0 items-center gap-1 rounded-[7px] border border-blue-100 bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700 hover:bg-blue-100"
        >
          Semua
      ...
      <div className="divide-y divide-slate-100">
        {unitActivities.map((activity) => (
          <Link
            ...
            key={activity.title}
            prefetch={false}
            className="-mx-2 grid grid-cols-[84px_minmax(0,1fr)] gap-3 rounded-[7px] px-2 py-2.5 transition hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#1f4fea] first:pt-0 last:pb-0"
          >
            <div
              aria-label={activity.imageAlt}
              className="relative h-20 overflow-hidden rounded-[7px] border border-[#dce3ed] bg-cover bg-no-repeat"
              role="img"
              style={{
            ...
            <div className="flex min-w-0 flex-col">
              ...
              <p className="mt-1 overflow-hidden text-[11px] font-semibold leading-4 text-slate-500 [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:2]">
                {activity.summary}
              </p>
              ...
              <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] font-semibold text-slate-600">
                ...
                <p className="flex items-center gap-1.5">
                  <CalendarDays size={12} className="shrink-0 text-[#1f4fea]" />
                  <span>{activity.date}</span>
                  <span
                    className={`ml-1 rounded-full border px-1.5 py-0.5 text-[9px] font-bold ${getActivityFunctionBadgeClass(activity.category)}`}
                  >
                    {activity.category}
                  </span>
                </p>
